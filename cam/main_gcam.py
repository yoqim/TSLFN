...
def process_train_sysu(data_path):
    ...
    new_dict = {v : k for k, v in pid2label.items()}
    cur_label = [134,15,10,31]
    for c in cur_label:
        print("changed label : ",c)
        print("origin label : ",new_dict[c])
...
def vis_cam(images, raw_images, model, id_label, target_layer, output_dir):
    ...
    for j in range(len(images)):
        # save_gradcam(filename=osp.join(output_dir,'rgas',"{}-gradcam_gt.png".format(sel_id[j])), gcam=regions[j,0], raw_image=raw_images[j])
        save_gradcam(filename=osp.join(output_dir,'right',"{}-gradcam.png".format(sel_id[j])), gcam=regions[j,0], raw_image=raw_images[j])
...
def get_bad_ids(images, model, id_label, target_layer):
    ...
    bad_id = bad_id.cpu().numpy() 
    bad_ids = [sel_id[i] for i in bad_id]
    
    print("bad_id len, ",len(bad_id))
    print("bad ids, ",bad_ids)
    
    print(pred_label[bad_id,:])
    print(id_label[bad_id])

    import pdb;pdb.set_trace()
    
    # f = open('bad_index.txt', 'w')
    # for i, bi in enumerate(bad_ids):
    #     f.write(str(bi))
    #     if i == len(bad_ids)-1:
    #         f.write('\n')
    #     else: 
...
# model_name = 'sysu_lr_1.0e-02_md_all_sharenet3_mulcla4_graph_RGAs4_best.t'
...
# Load model
model = embed_net_graph(low_dim, n_class, height=img_h, width=img_w, npart=npart, share_net=share_net, branch_name=None)
model = model.cuda()
...
# sel_id = [1847,11471,7604,19773,9397,10734,17332,18325,1261,4913,16114,20622,18509,13968,18166,4669,19083,12664,20074,2343,13845,9906,17742,3575]

# sel_id = [2343,4669,9397,9906,10734,11471,13968,17742,19083]
sel_id = [20599, 14753, 13920, 5482, 8081, 18574, 18310, 16462, 21014, 12097, 14830, 12421, 1988, 9187, 80, 16045, 19577, 10742,5567, 14787, 2270, 709, 11087, 5956]
